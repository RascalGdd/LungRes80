...
def auto_load_model(
    ...
    if loss_scaler is not None:
        ...
        # DeepSpeed explicit resume supports checkpoints outside the new
        # output directory (for example, seeding a prediction-bank run from a
        # visual-only checkpoint-best).
        if args.resume:
            resume_path = Path(args.resume).resolve()
            if not resume_path.is_dir():
                raise FileNotFoundError(
                    f"DeepSpeed resume checkpoint directory not found: {resume_path}"
                )
            load_path, client_states = model.load_checkpoint(
                str(resume_path.parent), tag=resume_path.name
            )
            if load_path is None:
                raise RuntimeError(f"Failed to load DeepSpeed checkpoint: {resume_path}")
            checkpoint_epoch = client_states.get("epoch")
            if isinstance(checkpoint_epoch, int) and args.start_epoch == 0:
                args.start_epoch = checkpoint_epoch + 1
            print(
                f"Resume DeepSpeed checkpoint {resume_path}; "
