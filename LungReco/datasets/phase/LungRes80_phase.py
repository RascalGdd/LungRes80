...
class PhaseDataset_LungRes80(Dataset):
    ...
    def __init__(
        self,
        anno_path="/path/to/LungRes80/labels/train/1fpstrain.pickle",
        data_path="/path/to/LungRes80",
        mode="train",  # val/test
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            metadata = self._metadata(
                ...
                frame_id,
                False,
                gt_recent_phases=frames_info.get("gt_recent_phases"),
                prediction_recent_phases=frames_info.get(
                    "prediction_recent_phases"
                ),
                prediction_recent_phase_times=frames_info.get(
                    "prediction_recent_phase_times"
                ),
            )
    ...
    def _metadata(
        ...
        frame_id,
        duplicate_frames,
        gt_recent_phases=None,
        prediction_recent_phases=None,
        prediction_recent_phase_times=None,
    ):
        metadata = {
            "duplicate_frames": bool(duplicate_frames),
        }
        if gt_recent_phases is not None:
            max_phases = 4  # Up to three transitions require four phase nodes.
            history = [int(phase_id) for phase_id in gt_recent_phases][-max_phases:]
            padded = history + [-1] * (max_phases - len(history))
            metadata["gt_recent_phases"] = torch.tensor(padded, dtype=torch.long)
            metadata["gt_recent_phase_count"] = len(history)
        if prediction_recent_phases is not None:
            max_phases = int(getattr(self.args, "transition_history_length", 3))
            history = [int(phase_id) for phase_id in prediction_recent_phases][
                -max_phases:
            ]
            padded = history + [-1] * (max_phases - len(history))
            metadata["prediction_recent_phases"] = torch.tensor(
                padded, dtype=torch.long
            )
            metadata["prediction_recent_phase_count"] = len(history)
            times = (
                [int(value) for value in prediction_recent_phase_times][
                    -max_phases:
                ]
                if prediction_recent_phase_times is not None
                else [-1] * len(history)
            )
            padded_times = times + [-1] * (max_phases - len(times))
            metadata["prediction_recent_phase_times"] = torch.tensor(
                padded_times, dtype=torch.long
            )
        return metadata
    ...
    def _aug_frame(
        ...
        if getattr(args, "paper_augmentation", False):
            # LungReco Sec. 6.1 reports random horizontal flip only.
            do_flip = random.random() < 0.5
            resize = transforms.Resize((self.crop_size, self.crop_size))
            frames = []
            for frame in buffer:
                image = resize(transforms.ToPILImage()(frame))
                if do_flip:
                    image = transforms.functional.hflip(image)
                frames.append(transforms.ToTensor()(image))
            buffer = torch.stack(frames).permute(0, 2, 3, 1)
    ...
    def _make_dataset(self, infos):
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                ...
                # Training samples may be randomly shuffled because their
                # causal phase context is precomputed from labels strictly
                # before the current frame.  At inference the same field is
                # populated online from prior model predictions instead.
                line_info["gt_recent_phases"] = list(recent_phases)
                frames.append(line_info)
                phase_id = int(line_info["phase_gt"])
                if not recent_phases or recent_phases[-1] != phase_id:
                    recent_phases.append(phase_id)
                    recent_phases = recent_phases[-4:]
        return frames
...
def build_dataset(is_train, test_mode, fps, args):
    ...
    print("%s %s - %s : Number of the class = %d" % ("LungRes80", mode, fps, args.nb_classes))
